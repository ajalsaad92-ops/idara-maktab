-- Migration: Add calculate_productivity_score function
-- Created: 2025-06-04
...
BEGIN
  ...
  WITH event_pairs AS (
    ...
    WHERE user_id = p_user_id
      AND DATE(event_at) = p_date
    ORDER BY event_at
  )
  SELECT
    COALESCE(SUM(
      CASE WHEN event_type = 'out' AND prev_event_type = 'in'
        THEN EXTRACT(EPOCH FROM (event_at - prev_event_at)) / 3600
        ELSE 0
      END
    ), 0),
    COALESCE(SUM(
      CASE WHEN event_type = 'in' AND prev_event_type = 'out'
        THEN EXTRACT(EPOCH FROM (event_at - prev_event_at)) / 3600
        ELSE 0
      END
    ), 0)
...
-- Also create a view for daily productivity reports
...
-- Add comment
COMMENT ON FUNCTION calculate_productivity_score IS 'Calculates daily productivity score: (completed_tasks × 10) + (hours_in × 2) - (hours_out × 1), capped at 0-100';
